	...
	public void perform() {
		...
		if (AccountHandler.exists(this.parameters.get(0)))
		{
			...
			if (Craftconomy.isValidAmount(this.parameters.get(1)))
			{
				...
				sendMessage("You removed " + Craftconomy.format(amount) + " from " + receiverAccount.getPlayerName() + " account!");
				sendMessage(receiverAccount.getPlayer(),  "" + ChatColor.RED + Craftconomy.format(amount) + " has been removed from your account!");
			}
			else
