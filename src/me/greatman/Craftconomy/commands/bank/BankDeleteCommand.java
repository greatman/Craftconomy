...
public class BankDeleteCommand extends BaseCommand{
	...
	public void perform() {
		...
		{
			if (BankHandler.getBank(this.parameters.get(0)).getOwner().equals(player.getName()) || player.hasPermission("Craftconomy.bank.admindelete"))
			{
				BankHandler.delete(this.parameters.get(0));
				sendMessage("The bank account " + ChatColor.WHITE + this.parameters.get(0) + ChatColor.GREEN +" has been deleted!");
